...
def get_repo(repo_name):
    ...
    return g.get_repo(f"{GITHUB_USERNAME}/{repo_name}")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main_menu":
        ...
        await query.edit_message_text(
            ...
            "*HTML ফাইলের জন্য:*\n"
            "GitHub Pages অটো চালু হবে 🌐\n\n"
            "*Python বট ফাইলের জন্য:*\n"
            "Render.com deploy লিংক পাবে 🚀",
            parse_mode="Markdown",
            reply_markup=back_keyboard()
        )
        ...
        user_sessions[query.from_user.id] = {"repo": repo_name, "action": "upload"}
        await query.edit_message_text(
    ...
    if user_id in user_sessions and user_sessions[user_id].get("action") == "new_repo":
        ...
        try:
            ...
            repo = auth_user.create_repo(
                repo_name,
                description="Created by GitHub Deploy Bot",
                private=False,
                auto_init=True
            )
...
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            f"✅ *{file_name} সফলভাবে {action} হয়েছে!*{extra}",
            parse_mode="Markdown",
            reply_markup=keyboard
        )
...
async def done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        text = "🎉 *সব ফাইল GitHub-এ push হয়ে গেছে!*\n\n"
...
async def health_server():
    """Render এর জন্য একটা ছোট HTTP server যেন port পায়"""
    ...
    await site.start()
    logger.info(f"Health server চালু হয়েছে port {port} এ")
